                        ...
                        <div class="col-6 col-xxl-3">
                          <div class="nav flex-column">
                            <p class="nav-link text-700 mb-0 fw-bold">Other Auth</p><a
                              ...
                              class="nav-link py-1 link-600 fw-medium" href="../index-2.html#authentication-modal"
            ...
            <div class="card bg-100 shadow-none border">
              <div class="row gx-0 flex-between-center">
                <div class="col-sm-auto d-flex align-items-center"><img class="ms-n2"
                  ...
                  <div>
                    <h6 class="text-primary fs-10 mb-0">Modification</h6>
                    <h4 class="text-primary fw-bold mb-0">Utilisateur<span class="text-info fw-medium"></span>
                    </h4>
...
if(isset($_GET['id'])) {
    ...
    if($requete->rowCount() > 0) {
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if(isset($_POST['mdp']) && isset($_POST['nom_utilisateur']) && isset($_POST['role'])) {
                // Récupérer le nouveau mot de passe, le nouveau nom et le nouveau rôle depuis le formulaire
                $nouveau_mdp = $_POST['mdp'];
                $nouveau_nom = $_POST['nom_utilisateur'];
                $nouveau_role = $_POST['role'];

                // Hasher le nouveau mot de passe
                $mdp_hash = password_hash($nouveau_mdp, PASSWORD_DEFAULT);

                // Requête SQL pour mettre à jour le mot de passe, le nom et le rôle de l'utilisateur
                $requete_update = $db->prepare("UPDATE user SET mdp = :mdp_hash, nom = :nouveau_nom, roles = :nouveau_role WHERE id = :id");
                $requete_update->bindParam(':mdp_hash', $mdp_hash);
                $requete_update->bindParam(':nouveau_nom', $nouveau_nom);
                $requete_update->bindParam(':nouveau_role', $nouveau_role);
                $requete_update->bindParam(':id', $id);
                $requete_update->execute();

                // Vérifier si la requête a réussi
                if($requete_update->rowCount() > 0) {
        ...
        // Afficher le formulaire pré-rempli avec les informations de l'utilisateur
?>
        <form class="form-admin" method="POST" action="">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="nom_utilisateur" name="nom_utilisateur" value="<?php echo $nom; ?>">
            </div>
            ...
            <div class="mb-3">
                ...
                <select class="form-select form-select-sm" id="role" name="role">
                    <?php
                        ...
                        $requete_roles = $db->query('SELECT * FROM roles');
                        while($row = $requete_roles->fetch()){
                            $selected = ($row['id_roles'] == $utilisateur['role']) ? 'selected' : ''; // Vérifier si le rôle correspond à celui de l'utilisateur
                            echo '<option value="'.$row['id_roles'].'" '.$selected.'>'.$row['nom_roles'].'</option>';
                        }
                    ?>
                </select>
            </div>
...
<?php
    ...
    echo "L'identifiant de l'utilisateur n'est pas spécifié dans l'URL.";
}
?>
